...
/**
 * Upload un fichier vidéo vers Cloudflare Stream DEPUIS NOTRE SERVEUR
 * (pas depuis le navigateur). Ça évite complètement les soucis de CORS
 * que Cloudflare impose sur les uploads directs depuis un navigateur.
 * Le fichier transite : navigateur → notre backend → Cloudflare.
 * Limite : ~200 Mo par fichier (méthode d'upload simple, pas TUS).
 */
async function uploadFileFromServer(filePath, fileName) {
  // Étape 1 : demander une URL d'upload à usage unique
  const uploadResponse = await client.post('/direct_upload', {
    maxDurationSeconds: 21600,
    requireSignedURLs: false,
  });
  const { uploadURL, uid: videoId } = uploadResponse.data.result;

  // Étape 2 : envoyer le fichier à cette URL, depuis notre serveur
